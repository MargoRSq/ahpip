...
def create_query_string_for_ahp_chart(
    ...
    for k, v in final_ahp['target_weights'].items():
        target_data.append(TargetWeights(comp=k, val=v))

    return query_string


def create_query_string_for_ahp_chart(labels: list[str], sizes: list[float]) -> str:
    query_params = {
        'labels': labels,
        'sizes': sizes,
    }
